...
public class BookingSearchService {
        ...
        public BookingResponseDto getAllBookings(Long userId, Pageable pageable) {
                ...
                Page<Booking> pastPage = bookingRepository.findByUserIdandStatus(userId,
                                BookingStatusEnum.PASTED.toString(),
                                pageable);
        ...
        public BookingCreateResponseDto createBooking(BookingCreateRequestDto bookingCreateRequestDto)  {
                // get user
                User user = userService.getUserInfo(bookingCreateRequestDto.getUsername());

                // get flight
                Flight flight = flightSearchService.getFlightById(Long.valueOf(bookingCreateRequestDto.getFlightId()));
                FlightDetailDto flightDetailDto = flightSearchService.getDetailById(flight.getFlight_id());
                ...
                booking.setReference("ref"+ flight.getFlight_id() +"-"+ RandomGenerator.getDefault().nextInt(1000));
                booking.setStatus(BookingStatusEnum.UPCOMING.toString());
                booking.setBooking_time(LocalDateTime.now());
                booking.setTotal_price(bookingCreateRequestDto.getTotalPrice());
                Booking bookingResult = bookingRepository.save(booking);

                return BookingCreateResponseDto.builder().bookingReference(bookingResult.getReference())
                                .booking_id(bookingResult.getBooking_id())
                                .route(concatString(flightDetailDto.getDeparture(), flightDetailDto.getDestination()))
                                .date(bookingResult.getBooking_time())
                                .build();
        }
